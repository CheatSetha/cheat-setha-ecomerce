...
const page = async ({params}) => {
  ...
  return (
    <div className='w-full'>
      ...
      {
        productByCategory.length <1 ? <h1 className='text-center'> there are no product available for {categoyById.name}</h1>  :
        productByCategory.map((product)=> {
          return (
      
              <Card key={product.id} title={product.title} desc={product.description} price={product.price} img={product.images[0]}/>
